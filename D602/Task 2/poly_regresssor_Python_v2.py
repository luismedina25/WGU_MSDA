...
parser.add_argument('--order', type=int, default=1, help='Polynomial feature order')
...
# configure logger
logname = "polynomial_regressionv2.txt"
...
logging.info("Flight Departure Delays Polynomial Regression Model Log Version 2")
logging.info(f"Parameters: num_alphas = {num_alphas}, order = {order}")
...
def create_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df2 = df[['SCHEDULED_DEPARTURE','SCHEDULED_ARRIVAL',
                                    'DEST_AIRPORT','DEPARTURE_DELAY']].copy()
...
# Load in the data
df = pd.read_csv("cleaned_datav2.csv")
...
with mlflow.start_run(experiment_id = experiment.experiment_id, run_name=run_name):
    ...
    for alpha in range(0, alpha_max, 2):
        ...
        # Add some improvements for version 2
...
Y = Y_test
tips = pd.DataFrame()
tips["prediction"] = pd.Series([float(s) for s in result.flatten()])
...
with mlflow.start_run(experiment_id = experiment.experiment_id, run_name = "Final Model - Test Data"):
    # 1. Log informational log files as artifacts
    mlflow.log_artifact("import_datav2.log")
    mlflow.log_artifact("clean_data_v2.log")

    # 2 Log input parameters
    mlflow.log_param("alpha", parameters[0] / 10)
    mlflow.log_param("order", parameters[1])

    # 3 Log performance plot
